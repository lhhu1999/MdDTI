...
if __name__ == '__main__':
    # dataset = 'dude_1_1', 'dude_1_3', 'dude_1_5'
    dataset = 'dude_1_1'
    os.makedirs('../datasets/interaction/' + dataset + '/train', exist_ok=True)
    os.makedirs('../datasets/interaction/' + dataset + '/test', exist_ok=True)

    print(">>> Encode in " + dataset)
    for task in ['train', 'test']:
        smiles_all, skeletons, marks, components, residues, interactions = [], [], [], [], [], []
        filename = "../RawData/interaction/{}/{}_shuffle.txt".format(dataset, task)
        path_output = "../datasets/interaction/{}/{}".format(dataset, task)
