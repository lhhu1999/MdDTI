    ...
    for atom in mol.GetAtoms():
        if len(atom.GetNeighbors()) > 2 and not atom.IsInRing():
            ...
            for nei in atom.GetNeighbors():
                if mol.GetBondBetweenAtoms(nei.GetIdx(), atom.GetIdx()).GetBondType().name == 'SINGLE' and nei.GetIsAromatic() is False:
...
if __name__ == "__main__":
    dataset = 'Kd'  # 'Kd', 'EC50'  (Note: Run once for each dataset)
    components_cmf = []
    path_output = "../datasets/affinity/{}".format(dataset)
    with open("../datasets/affinity/{}/smiles.txt".format(dataset), "r") as f:
    ...
    with open('../datasets/affinity/' + dataset + '/drugs_cmf_dict', 'wb') as f:       # 保存子图字典
